...
CASE_STUDY_DIR = Path("case-studies/linear-regression-models/football-points-prediction")
...
def pretty_print(name: str, m: RegressionMetrics) -> None:
    logger.info("\n=== %s ===", name)
    logger.info("MAE:   %s  (avg absolute error in points)", f"{m.mae:.3f}")
    logger.info("RMSE:  %s (penalizes large misses)", f"{m.rmse:.3f}")
    logger.info("MSE:   %s  ((points)^2)", f"{m.mse:.3f}")
    logger.info("R^2:   %s   (variance explained)", f"{m.r2:.3f}")
    logger.info("SMAPE: %s (stable relative error)", f"{m.smape:.3f}")
